...
fn draw_spectrum(
    ...
    let fq_end =
        (((analyzer_data.num_bins - 1) as f32 * sr) / (analyzer_data.num_bins * 2) as f32).ln();
    let range = fq_end - fq_start;

    let bin_x = |bin_idx: f32| (bin_frequency(bin_idx).ln() - fq_start) / range;

    let magnitude_height = |magnitude: f32| db_to_unclamped_height(magnitude).clamp(0.0, 1.0);
...
pub fn draw_cutoffs(
    ...
    let freq_to_x = |f: f32| (f.ln() - fq_start) / range;
